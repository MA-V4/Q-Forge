...
use qforge_optimizer::{
    ...
    IdentityElimination, RotationMerging, PassManager,
};
...
impl CostFunction {
    pub fn score(&self, strategy: &CompilationStrategy, baseline_gates: usize) -> f64 {
        ...
        + self.gate_weight   * gate_score
        - two_q_penalty
    }
}

/// Run multiple compilation strategies, score each, return ranked results.
pub fn evaluate_strategies(circuit: &Circuit, noise: &NoiseModel) -> Vec<(CompilationStrategy, f64)> {
...
fn build_strategies(circuit: &Circuit) -> Vec<CompilationStrategy> {
    ...
    configs.into_iter().map(|(name, passes)| {
        let mut pm = PassManager::new();
        for pass in passes { pm.add_pass_boxed(pass); }
