...
impl Pass for CommutationAnalysis {
    fn name(&self) -> &str { "commutation_analysis" }
    ...
    fn run(&self, mut circuit: Circuit) -> (Circuit, PassReport) {
        ...
        while changed {
            ...
            while i < n {
                ...
                while j < n && j < i + 8 { // limit lookahead to 8 gates
                    if skip.contains(&j) { j += 1; continue; }
                    ...
                    if !commutes(&gates[i], &gates[j]) { break; }

                    j += 1;
                }

                if !found { out.push(gates[i].clone()); }
                i += 1;
            }
...
pub fn commutes(a: &Gate, b: &Gate) -> bool {
    ...
    if qa.is_disjoint(&qb) { return true; }
