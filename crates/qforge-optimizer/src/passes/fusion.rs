...
fn r(re: f64)          -> Complex64 { Complex64::new(re, 0.0) }
...
fn gate_matrix(gate: &Gate) -> Option<M2> {
    ...
    Some(match gate {
        ...
        Gate::U1(l, _) => {
            [[r(1.0), r(0.0)], [r(0.0), c(l.cos(), l.sin())]]
        }
        Gate::U3(t, p, l, _) => {
            let cos = (t / 2.0).cos();
            let sin = (t / 2.0).sin();
            [
                ...
                [c(sin * p.cos(), sin * p.sin()), c(cos * (p + l).cos(), cos * (p + l).sin())],
            ]
        }
...
fn mat_to_gate(mat: M2, qubit: QubitRef) -> Option<Gate> {
    ...
    if theta.abs() < 1e-10 {
        ...
        if angle.abs() < 1e-10 { return None; }
...
fn single_qubit_target(gate: &Gate) -> Option<QubitRef> {
    match gate {
        ...
        | Gate::U1(_, q) | Gate::U3(_, _, _, q) => Some(q.clone()),
        _ => None,
    }
}
...
fn is_two_qubit(gate: &Gate) -> bool {
    matches!(gate, Gate::Cx(_, _) | Gate::Cz(_, _) | Gate::Swap(_, _) | Gate::Ccx(_, _, _))
}