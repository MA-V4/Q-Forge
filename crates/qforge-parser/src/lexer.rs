...
impl Lexer {
        ...
        loop {
            match self.advance() {
                None | Some('\n') => return Err(ParseError::UnexpectedToken {
                    line: self.line, message: "unterminated string".into(),
                }),
                Some('"') => break,
                Some(c)   => s.push(c),
            }
    ...
    fn read_number(&mut self) -> Token {
        ...
        while let Some(c) = self.peek() {
            ...
            else if c == '.' && !is_real { s.push(c); is_real = true; self.advance(); }
            else if (c == 'e' || c == 'E') && !s.contains('e') {
                s.push(c); is_real = true; self.advance();
    ...
    pub fn tokenise(&mut self) -> Result<Vec<Token>, ParseError> {
        ...
        loop {
            ...
            match self.peek() {
                ...
                Some('(')  => { self.advance(); out.push(Token::LParen); }
                Some(')')  => { self.advance(); out.push(Token::RParen); }
                Some('[')  => { self.advance(); out.push(Token::LBracket); }
                Some(']')  => { self.advance(); out.push(Token::RBracket); }
                Some('{')  => { self.advance(); out.push(Token::LBrace); }
                Some('}')  => { self.advance(); out.push(Token::RBrace); }
                Some(';')  => { self.advance(); out.push(Token::Semicolon); }
                Some(',')  => { self.advance(); out.push(Token::Comma); }
                Some('+')  => { self.advance(); out.push(Token::Plus); }
                Some('*')  => { self.advance(); out.push(Token::Star); }
                Some('^')  => { self.advance(); out.push(Token::Caret); }
                Some('/') => { self.advance(); out.push(Token::Slash); }
                Some('-') => {
                    self.advance();
                    if self.peek() == Some('>') { self.advance(); out.push(Token::Arrow); }
                    else { out.push(Token::Minus); }
                }
                Some('=') => {
                    self.advance();
                    if self.peek() == Some('=') { self.advance(); out.push(Token::Equals); }
                    else {
                        return Err(ParseError::UnexpectedToken {
                            line, message: "expected '==' not '='".into(),
                        });
                    }
                }
                Some('"') => {
                ...
                Some(c) => {
                    ...
                    self.advance();
                    return Err(ParseError::UnexpectedToken {
                        line, message: format!("unexpected character '{}'", ch),
                    });
                }
            }
        }
