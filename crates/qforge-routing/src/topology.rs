...
impl HardwareTopology {
    pub fn are_adjacent(&self, q0: usize, q1: usize) -> bool {
        self.coupling_map.iter().any(|e| {
            (e.source == q0 && e.target == q1) ||
            (e.source == q1 && e.target == q0)
        })
    }

    pub fn neighbours(&self, qubit: usize) -> Vec<usize> {
        self.coupling_map.iter().flat_map(|e| {
            if e.source == qubit { Some(e.target) }
            else if e.target == qubit { Some(e.source) }
            else { None }
        }).collect()
    }
    ...
    pub fn distance(&self, src: usize, dst: usize) -> usize {
        ...
        while let Some((node, dist)) = queue.pop_front() {
            for nb in self.neighbours(node) {
                ...
                if visited.insert(nb) { queue.push_back((nb, dist + 1)); }
            }
        }
    ...
    pub fn distance_matrix(&self) -> Vec<Vec<usize>> {
        ...
        (0..n).map(|i| (0..n).map(|j| self.distance(i, j)).collect()).collect()
    }
    ...
    pub fn linear(n: usize) -> Self {
        Self {
            ...
            qubit_count: n,
            coupling_map: (0..n - 1)
                .map(|i| CouplingEdge { source: i, target: i + 1 })
                .collect(),
    ...
    pub fn grid(rows: usize, cols: usize) -> Self {
        ...
        for r in 0..rows {
            for c in 0..cols {
                let idx = r * cols + c;
                if c + 1 < cols { edges.push(CouplingEdge { source: idx, target: idx + 1 }); }
                if r + 1 < rows { edges.push(CouplingEdge { source: idx, target: idx + cols }); }
            }
        }
        Self {
    ...
    pub fn heavy_hex(n: usize) -> Self {
        ...
        for i in (0..n.saturating_sub(1)).step_by(2) {
            edges.push(CouplingEdge { source: i, target: i + 1 });
            if i + 2 < n { edges.push(CouplingEdge { source: i + 1, target: i + 2 }); }
            if i + 3 < n { edges.push(CouplingEdge { source: i + 2, target: i + 3 }); }
        }
        Self {
