...
pub fn route(circuit: &Circuit, topology: &HardwareTopology, initial_mapping: &[usize]) -> (Circuit, RoutingReport) {
    ...
    for gate in &circuit.gates {
        match gate {
            Gate::Cx(ctrl, tgt) | Gate::Cz(ctrl, tgt) => {
                let lc = *logical_index.get(&format!("{}_{}", ctrl.register, ctrl.index)).unwrap_or(&0);
                let lt = *logical_index.get(&format!("{}_{}", tgt.register, tgt.index)).unwrap_or(&1);
                ...
                if topology.are_adjacent(pc, pt) {
                    ...
                    for window in path.windows(2) {
                        ...
                        if pa == pc && pb == pt { break; }

                        // Insert SWAP between pa and pb
                        routed_gates.push(Gate::Swap(
                            phys_qubit_ref(pa),
                            phys_qubit_ref(pb),
                        ));
                        swaps_inserted += 1;
                        ...
                        if la != usize::MAX { log_to_phys[la] = pb; }
                        if lb != usize::MAX { log_to_phys[lb] = pa; }
                        phys_to_log[pa] = lb;
                        phys_to_log[pb] = la;
                    }
...
fn remap_gate(gate: &Gate, log_to_phys: &[usize], logical_index: &HashMap<String, usize>) -> Gate {
    let remap = |q: &QubitRef| -> QubitRef {
        let l = *logical_index.get(&format!("{}_{}", q.register, q.index)).unwrap_or(&0);
        let p = if l < log_to_phys.len() { log_to_phys[l] } else { l };
        phys_qubit_ref(p)
    };
    ...
    match gate {
        ...
        Gate::Custom { name, params, qubits } => Gate::Custom {
            name:   name.clone(),
            params: params.clone(),
